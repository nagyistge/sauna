...
class DcpResponderTest extends FunSuite with BeforeAndAfter  {
  ...
  test("correct line") {
    ...
    assert(dcpDatasource.correctLine(line1).contains("true,123,alpha,1387116306789"))
    assert(dcpDatasource.correctLine(line2).contains("false,456,delta,1402436912712"))
    assert(dcpDatasource.correctLine(line3).contains("false,789,omega,1422430336329"))
  }
  ...
  test("correct line with invalid date") {
    ...
    assert(dcpDatasource.correctLine(line).contains("falsetrue,123,alpha,2013qqqqq12-15 14:05:06.789"))
  }
  ...
  test("correct line with wrong date") { // let this be a feature
    ...
    assert(dcpDatasource.correctLine(line).contains("true,123,alpha,1615817106789"))
  }
